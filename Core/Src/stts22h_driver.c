...
STTS22H_Status stts22h_configure(STTS22H_Mode mode) {
  ...
  if (mode == STTS22H_MODE_FREERUN_25HZ 
    ...
    || mode == STTS22H_MODE_FREERUN_100HZ 
    || mode == STTS22H_MODE_FREERUN_200HZ) {
    // TODO 7a: Set FREERUN bit to 1:
    reg_ctrl |= (0x1 << 2);
  } else {
    // TODO 7b: Set FREERUN bit to 0:
    reg_ctrl &= ~(0x1 << 2);
  }
...
STTS22H_Status stts22h_trigger_oneshot(void) {
  // TODO 1: Read current "CTRL" register:
  uint8_t reg_ctrl = 0;
  if(HAL_I2C_Mem_Read(&hi2c1, STTS22H_I2C_ADDR_7B << 1, STTS22H_REG_CTRL, I2C_MEMADD_SIZE_8BIT, &reg_ctrl, 1, HAL_MAX_DELAY) != HAL_OK) {
    return STTS22H_ERROR;
  }
  ...
  // TODO 4: Write new CTRL register value to sensor to apply it:
    if(HAL_I2C_Mem_Write(&hi2c1, STTS22H_I2C_ADDR_7B << 1, STTS22H_REG_CTRL, I2C_MEMADD_SIZE_8BIT, &reg_ctrl, 1, HAL_MAX_DELAY) != HAL_OK) {
      return STTS22H_ERROR;
    }
...
STTS22H_Status stts22h_check_is_busy(bool *is_busy) {
  ...
  if(HAL_I2C_Mem_Read(&hi2c1, STTS22H_I2C_ADDR_7B << 1, STTS22H_REG_STATUS, I2C_MEMADD_SIZE_8BIT, &reg_status, 1, HAL_MAX_DELAY) != HAL_OK) {
    return STTS22H_ERROR;
  }
