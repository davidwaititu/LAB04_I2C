...
void test_driver(void) {
  ...
  if(stts22h_configure( STTS22H_MODE_FREERUN_25HZ) == STTS22H_OK){
    printf("Sensor configured to free-run mode at 25Hz output data rate!\r\n");
  } else {
  ...
  for (unsigned int i = 0; i < 200; i++) {
    ...
    // TODO: Use your stts22h_read_temp() function to retrieve the
    ...
    //       the Error_Handler() function.
      if(stts22h_read_temp(&temp_centidegree) != STTS22H_OK){
        printf("Failed to read temperature measurement from sensor!\r\n");
        Error_Handler();
      }

    // TODO: Convert temp_centidegree value to actual degrees:
    float temp_degress = 0;
    temp_degress = (float)temp_centidegree / 100.0f;
  ...
  if(stts22h_configure( STTS22H_MODE_ONE_SHOT) == STTS22H_OK){
    printf("Sensor configured to one-shot mode!\r\n");
  } else {
    printf("Sensor configuration ERROR!\r\n");
    Error_Handler();
  }
  ...
  while (1) {
    ...
    if(stts22h_trigger_oneshot() == STTS22H_OK){
      printf("One-shot measurement triggered!\r\n");
    } else {
      printf("Failed to trigger one-shot measurement!\r\n");
      Error_Handler();
    }
...
if (stts22h_check_is_busy(&is_busy) != STTS22H_OK) {
  printf("Failed to check if sensor is busy!\r\n");
  Error_Handler();
}
      if (!is_busy) {
    ...
    float temp_degrees = (float)temp_centidegree / 100.0f; 
    printf("One-shot measurement: %.2f °C\r\n", temp_degrees);
